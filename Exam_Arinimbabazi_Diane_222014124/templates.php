...
<body style="background-image: url('./Images/tempp.jpg');background-repeat: no-repeat;background-size:cover;">
  <!-- Header section -->
  <header>
    <h1>Templates</h1>
  </header>
  ...
  <section>
    ...
    <form method="post" onsubmit="return confirm('Are you sure you want to insert this record?');">
      ...
      <input type="number" id="template_id" name="template_id"><br><br>
    ...
    <?php
      ...
      if ($_SERVER["REQUEST_METHOD"] == "POST") {
          ...
          $stmt = $connection->prepare("INSERT INTO templates (template_id, name, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?)");
          $stmt->bind_param("iisss", $template_id, $name , $description, $created_at, $updated_at);

          // Set parameters from POST data with validation (optional)
          $template_id = intval($_POST['template_id']); // Ensure integer for template_id
          $name= intval($_POST['name']); // Ensure integer for name
      ...
      <?php
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $template_id = $row['template_id']; // Update template_id variable
                echo "<tr>
                  ...
                  <td><a style='padding:4px' href='Delete_templates.php?template_id=$template_id'>Delete</a></td> 
                  <td><a style='padding:4px' href='update_templates.php?template_id=$template_id'>Update</a></td> 
                </tr>";
            }
        } else {
